...
| 智能体运营中心 | “AI 智能体” (`/teacher/agents`, `/teacher/agents/manage`, `/teacher/agent/:id`) | **新增**：创建/复制智能体；**修改**：编辑画像/能力、上下架；**删除**：逻辑删除智能体；**查询/监控**：大盘使用/评分/评论趋势、学段覆盖度；**运营**：评论批量审核（多选、话术模板、备注）、置顶推荐；**导出/导入**：大盘多 Sheet Excel 导出，单个智能体支持学生使用明细 Excel 导入/导出（含模板下载） | 表 `agent_profiles`, `agent_statistics`, `agent_comments`, `agent_usage_records`, `agent_templates`, `agent_usage_import_jobs`, `agent_comment_audit_logs`。 |
...
- **智能体运营接口**：`POST /agents`, `PUT /agents/{id}`, `GET /agents/statistics`, `POST /agent-usage/import`（Excel 上传作业）、`GET /agent-usage/export`、`POST /agent-comments/audit`（批量审核，含备注）。  
...
- **教师端**（Demo 左侧“教师系统”）  
  ...
  - 智能体运营中心 `#/teacher/agents`、`#/teacher/agent/:id`、`#/teacher/agent/:id/comments`  
    - 操作：创建/复制/上下架智能体；查看大盘使用/评分/评论与学段覆盖度；评论多选批量审核（话术模板+备注）；学生使用明细 Excel 导入/导出、模板下载；大盘多 Sheet Excel 导出  
    - 结果：`agent_profiles`, `agent_statistics`, `agent_comments`, `agent_usage_records`, `agent_usage_import_jobs`, `agent_comment_audit_logs` 持续更新，学生端广场/对话同步展示
...
7. **智能体运营闭环**：教师端可创建/上下架智能体、查看使用/评分/评论/学段覆盖趋势、批量审核评论（多选+话术模板+备注）、导出大盘多 Sheet Excel、导入/导出单体学生使用明细，学生端可搜索/对话/评分，数据沉淀在 `agent_profiles` + `agent_statistics` + `agent_comments` + `agent_usage_import_jobs` 形成可运营的数字老师生态。
